        ...
        match qsketch_core::io::ase::parse(&bytes) {
            Ok(s) => {
                println!(
                    ...
                    s.width, s.height, s.depth, s.frames.len(), s.layers.len(), s.tags.len(), s.palette.len()
                );
                for l in &s.layers {
                    println!("  layer {:?} kind={} level={} blend={} op={} flags={:#x}", l.name, l.kind, l.child_level, l.blend, l.opacity, l.flags);
                }
                ...
                match s.to_doc(0) {
                    Ok((d, w)) => {
                        ...
                        let same = back.layers.len() == d.layers.len()
                            && back.layers.iter().zip(&d.layers).all(|(a, b)| a.props.name == b.props.name && a.raster.to_rgba() == b.raster.to_rgba());
