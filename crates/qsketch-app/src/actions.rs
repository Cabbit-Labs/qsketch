...
actions! {
    ...
    ExportScaled2 => (File, "Export at 2×…", []),
    ...
    SelectLayerAbove => (Layer, "Select Layer Above", ["Shift+W"]),
    SelectLayerBelow => (Layer, "Select Layer Below", ["Shift+S"]),
    ...
    Reselect => (Select, "Reselect", ["Ctrl+Shift+D"]),
    FeatherSelection => (Select, "Feather…", ["Shift+F6"]),
    ExpandSelection => (Select, "Expand…", []),
    ContractSelection => (Select, "Contract…", []),
    BorderSelection => (Select, "Border…", []),
    SmoothSelection => (Select, "Smooth…", []),
    SharpenSelection => (Select, "Sharpen", []),
    RemoveSelectionHoles => (Select, "Remove Holes", []),
    ...
    TiledOff => (View, "Tiled: Off", []),
    TiledX => (View, "Tiled: Across", []),
...
impl Action {
    ...
    pub fn tool(self) -> Option<ToolKind> {
        Some(match self {
            ...
            Action::ToolPolyLasso => ToolKind::PolyLasso,
            Action::ToolMagicWand => ToolKind::MagicWand,
            Action::ToolSelectBrush => ToolKind::SelectBrush,
            ...
            Action::ToolSmudge => ToolKind::Smudge,
            Action::ToolClone => ToolKind::Clone,
    ...
    pub fn for_tool(tool: ToolKind) -> Action {
        match tool {
            ...
            ToolKind::PolyLasso => Action::ToolPolyLasso,
            ToolKind::MagicWand => Action::ToolMagicWand,
            ToolKind::SelectBrush => Action::ToolSelectBrush,
            ...
            ToolKind::Smudge => Action::ToolSmudge,
            ToolKind::Clone => Action::ToolClone,
